    ...
    stages {
        ...
        stage('Gradle Build') {
            steps {
                ...
                dir('drill') {
                    ...
                    sh "./gradlew clean build -x test -Pspring.profiles.active=${PROFILE}" // 프로파일 전달'
                }
            }
        }
        ...
        stage('Docker Build') {
            steps {
                dir('drill') {
                    // 이미지를 빌드합니다.
                    sh 'docker build -t drill:${PROFILE} .'
                }
            }
        }
        ...
        stage('Deploy') {
            steps {
                ...
                sh 'docker run -d --name back -p 8060:8060 -u root drill:${PROFILE}'
            }
        }
